...
Rules: `name` is kebab-case; every `source`/`run` resolves inside the pack (no `..`); `${secrets.x}` / `${options.x}` references are interpolated at install time and unresolved references throw; hook events are `SessionStart`. Adapters carry pack capabilities through the optional `Adapter.pack` writer set (`mcp` / `skill` / `lifecycleHook` + removals) — an adapter without a writer skips that capability, reported, never fatal.
