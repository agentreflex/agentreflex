...
- **session hooks** — scripts that run at session boundaries (`SessionStart`)
...
arx add memcell                         # by name, from the registry
```
...
```json pack.json
{
  ...
  "secrets": {
    "memcell_token": {
      "title": "MemCell token",
      "description": "memcell.ai → Settings → Connect"
    }
  },
  "options": {
    "memcell_url": { "title": "MCP endpoint", "default": "https://api.memcell.ai/mcp" }
  },
  "mcp": {
    "memcell": {
      "type": "http",
...
```
● pack memcell/memcell  connected
● pack memcell/memcell  unauthorized   token rejected (401) — re-run arx add to update it
● pack memcell/memcell  host-rejected  server rejected the Host header (421) — its allowed-hosts list is missing this hostname:port
● pack memcell/memcell  unreachable    no response — is the server up?
```
