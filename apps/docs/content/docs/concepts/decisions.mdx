...
Claude Code, Cursor, and Copilot CLI honor both `deny` and `ask`. Gemini CLI, Windsurf, and OpenCode honor `deny` — an `ask` there isn't enforced and the call is allowed through, so prefer `deny` for rules that must hold everywhere. See the [capability matrix](/docs/concepts/capability-matrix).
