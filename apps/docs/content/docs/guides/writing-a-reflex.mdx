...
## Reacting to results with `onToolResult`

`onToolResult` runs *after* a tool. Use it for snapshots, logging, or post-hoc checks — or return a [reaction](/docs/reference/spec#reaction) to feed something back to the agent: `inject(context)` adds context to its conversation, `block(reason)` rejects the result.

```ts
import { defineReflex, inject } from "@agentreflex/core";

export default defineReflex({
  name: "audit-writes",
  onToolResult(ctx) {
    if (ctx.tool === "Write") log(ctx.paths);
    if (ctx.tool === "Bash" && /EADDRINUSE/.test(ctx.output ?? ""))
      return inject("that port is already taken — a dev server is likely running.");
  },
});
```

Returning nothing is fine — the reflex is then a pure observer. On agents whose adapter can't carry reactions they degrade to no-ops.
...
- `--event onToolResult` — run the post-tool handler instead of the pre-tool check; add `--output "<text>"` to simulate the tool's output and see the reaction.
